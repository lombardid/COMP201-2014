...
enum State { INITIAL, FIRST, NO_MATCH };
...
Model::Model(int w, int h) {
	...
	for (int i = 0; i< height; i++)
	{
		for(int j = 0; j <width; j++)
		{
			...
			// replace grid with visible to see the randomization taking place
		}
	}
}
...
void Model::flip(int row, int column) {
	...
	case INITIAL:
		...
		grid[row][column]= last_val; 
		state=FIRST;
		break;
	case FIRST:
		visible[row][column]=grid[row][column];
		if(grid[row][column]!=last_val)
		{
			state=NO_MATCH;
		}
		else
		...
		break;
	case NO_MATCH:
		grid[row][column] = visible[row][column];
		state=INITIAL;
		break;
	}
